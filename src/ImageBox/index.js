...
class ImageBox extends React.Component {
  ...
  componentWillReceiveProps(props) {
    ...
    if (scale !== 0) {
      ...
      this.transform.scale = Math.max(
        0.1,
        this.transform.scale + (scale > 0 ? 0.1 : -0.1)
      );
      this.draw();
    }
  }
  ...
  draw = () => {
    ...
    ctx.translate(cw / 2, ch / 2); // 移动画布原点
    ctx.translate(this.transform.translateX, this.transform.translateY);
    ctx.rotate((this.transform.rotate * Math.PI) / 2); // 旋转画布
    ctx.scale(this.transform.scale, this.transform.scale); // 缩放画布
    ctx.drawImage(this.image, 0, 0, sw, sh, dx, dy, sw, sh); // 画图
    ctx.restore(); // 恢复画布到save以前
  };
  ...
  render() {
    ...
    return (
      <div style={{ height: height }}>
        <div
          className="image-previewer-watermark"
          ref={(instance) => {
            this.watermarkWrapper = instance;
          }}
        >
          <div
            style={{
        ...
        {!isError && isComplete && (
          ...
          >
            <canvas
              ...
              onContextMenu={(e) => e.preventDefault()} // 阻止默认事件来保存图片
              ref={(instance) => (this.canvas = instance)}
            />
          </div>
        )}
        {isComplete && isError && (
          <span className="image-previewer-loading-tips">{errorText}</span>
        )}
      </div>
    );
  }
